...
```{r}
##### Skills & Company for Database
skill_company <-data.frame(skillid=character(),companyid=character())

test <-job_df %>% select (company_name,inferred_skills)%>%
...
for (i in 1:datarow){
  ...
  rows<-data.frame(skillid=infer_byrow,companyid=test[[i,1]])
